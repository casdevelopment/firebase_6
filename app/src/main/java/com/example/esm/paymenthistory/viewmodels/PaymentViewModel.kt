...
class PaymentViewModel(private val repository: Repository): ViewModel() {
    ...
    fun downloadFeeReceipt(model: PaymentHistoryPostModel) :LiveData<NetworkStates<Response<String>>>{
        return liveData(Dispatchers.IO){
            emit(NetworkStates.loading(null))
            try {
                emit(NetworkStates.success(data = repository.downloadFeeReceipt(model)))
