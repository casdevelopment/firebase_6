package com.example.esm.hostel.viewmodels
...
class HostelViewModel(private val repository: Repository):ViewModel() {
    fun studentHostelInfo(model: IdentityModel): LiveData<NetworkStates<Response<StudentResponseModel>>> {
        return liveData(Dispatchers.IO){
            emit(NetworkStates.loading(null))
            try {
                emit(NetworkStates.success(data = repository.studentHostelInfo(model)))
