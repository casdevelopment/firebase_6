...
class DiaryViewModel(private val repository: Repository) : ViewModel() {
    ...
    fun getDiaryById(request : StudentDiaryRequestModel): LiveData<NetworkStates<Response<DiaryResponseModel>>> {
        return liveData(Dispatchers.IO){
            emit(NetworkStates.loading(null))
            try {
                emit(NetworkStates.success(data = repository.getDiaryById(request)))
